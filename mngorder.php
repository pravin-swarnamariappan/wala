...
<?php
    ...
    if($p == 'deliver' && isset($_GET['dispatch'])){
        $date = date('m/d/y h:i:s A');
        $id = $_GET['id'];
        $dispatch = $_GET['dispatch'];
        $q = "UPDATE orders set datedelivered='$date', status='delivered', dispatch_id='$dispatch' where id=$id";
        if ($con->query($q) === TRUE) {
			...
			<div class="row">
                ...
                <div class="col-sm-9 padding-right">
					...
					<div class="features_items"><!--features_items-->
                        ...
                        <li><a href="generateorders.php" class="btn-info">Generate PDF</a></li>
                        </ul>
                                ...
                                <?php  
                                    ...
                                    <tr>
                                        ...
                                        <form method="$_GET" action="" >
                                            <?php $result = $con->query('SELECT  * FROM dispatch');?>
                                            <select name="dispatch" required>
                                                <option value="">Select Dispatch<option>
                                                <?php
                                                    while($row2 = mysqli_fetch_array($result)){ ?>
                                                    <option value="<?php echo $row2['id'];?>"><?php echo $row2['name'];?> (<?php echo $row2['type'];?>)</option>
                                                <?php }?>
                                            </select>
                                            <input type="hidden" name="id" value="<?php echo $row['id']; ?>"/>
                                            <input type="submit" name="p" class="btn btn-warning" value="deliver"/></td>
                            ...
                            <table class="table table-bordered">
                                ...
                                <?php 
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center"><a href="order.php?p=paid&&id=<?php echo $row['id']; ?>" class="btn btn-warning">Pay</a></td>
                                    </tr>
                                <?php } ?>
                            </table>
                        </div>
